...
class PCAUnprojector(torch.nn.Module):

    def __init__(self, feats, dim, device, use_torch_pca=False):
        super().__init__()
        self.dim = dim
        self.original_dim = feats.shape[1]
        if self.dim != self.original_dim:
            sklearn_pca = pca([feats], dim=dim, use_torch_pca=use_torch_pca)[1]
            self.components_ = torch.tensor(sklearn_pca.components_, device=device, dtype=feats.dtype)
            self.singular_values_ = torch.tensor(sklearn_pca.singular_values_, device=device, dtype=feats.dtype)
            self.mean_ = torch.tensor(sklearn_pca.mean_, device=device, dtype=feats.dtype)
        else:
